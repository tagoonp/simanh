      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ...
          <th style="text-align:center;"  class="col_<?php print $value['hos_id'];?>">
            <?php
              $val = calculateTotal_N($db, $value['hos_id'], 8);
              print $val;
            ?>
          </th>
          <?php
      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ...
          <td  class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],2,7); ?>
          </td>
          <?php
        }
      }
    ...
    <tr>
      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ?>
          <td class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],3,8); ?>
          </td>
          <?php
        }
      }
    ...
    <tr>
      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ?>
          <td class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],4,9); ?>
          </td>
          <?php
        }
      }
    ...
    <tr>
      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ?>
          <td class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],5,10); ?>
          </td>
          <?php
        }
      }
    ...
    <tr>
      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ?>
          <td  class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],10,11); ?>
          </td>
          <?php
        }
      }
...
function calculate($db, $hos,$qs,$qsn){
	...
	$strSQL = "SELECT count(tb_inddata.ind_id) as a FROM tb_indicator8 inner join tb_inddata on tb_indicator8.ind_id=tb_inddata.ind_id where tb_indicator8.ind8_datediag between '" .$sd. "' and '".$ed."' and tb_inddata.hos_id = '".$hos."'";
	$resultTotal = $db->select($strSQL,false,true);
	$total = 0;
	if($resultTotal){
		$total = $resultTotal[0]['a'];
	...
	$strSQL = "SELECT COUNT( tb_inddata.ind_id ) AS a
				FROM tb_response8
				INNER JOIN tb_inddata ON tb_response8.ind_id = tb_inddata.ind_id
				WHERE tb_inddata.hos_id =  '".$hos."'
				AND tb_inddata.dadel
				BETWEEN  '".$sd."'
				AND  '".$ed."'
				AND tb_response8.qares".$qsn." =  '1'";
