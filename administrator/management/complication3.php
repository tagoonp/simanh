...
<table class="table">
      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ...
          <th style="text-align:center;"  class="col_<?php print $value['hos_id'];?>">
            <?php
              $val = calculateTotal_N($db, $value['hos_id'], 3);
              print $val;
            ?>
          </th>
          <?php
  ...
  <tbody>
      ...
      if($resultHospital){
        foreach($resultHospital as $value){
          ...
          <td  class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],1,27); ?>
          </td>
          <?php
        }
      }
    ...
    <tr>
      <td style="text-align: left;">
        2. Sepsis with associated organ failure
      </td>
      <?php
      if($resultHospital){
        foreach($resultHospital as $value){
          ?>
          <td class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],2,28); ?>
          </td>
          <?php
        }
      }
    ...
    <tr>
      <td style="text-align: left;">
        3. Sepsis with hypetension refractory resuscitation
      </td>
      <?php
      if($resultHospital){
        foreach($resultHospital as $value){
          ?>
          <td class="col_<?php print $value['hos_id'];?>">
            <?php calculate($db, $value['hos_id'],3,29); ?>
          </td>
          <?php
        }
      }
...
function calculate($db, $hos,$qs,$qsn){
	...
	$strSQL = "SELECT count(tb_inddata.ind_id) as a FROM tb_indicator3 inner join tb_inddata on tb_indicator3.ind_id=tb_inddata.ind_id where tb_indicator3.ind3_datediag between '" .$sd. "' and '".$ed."' and tb_inddata.hos_id = '".$hos."'";
	$resultTotal = $db->select($strSQL,false,true);
	$total = 0;
	if($resultTotal){
		$total = $resultTotal[0]['a'];
	...
	$strSQL = "SELECT COUNT( tb_inddata.ind_id ) AS a
				FROM tb_response3
				INNER JOIN tb_inddata ON tb_response3.ind_id = tb_inddata.ind_id
				WHERE tb_inddata.hos_id =  '".$hos."'
				AND tb_inddata.dadel
				BETWEEN  '".$sd."'
				AND  '".$ed."'
				AND tb_response3.qares".$qsn." =  '1'";
